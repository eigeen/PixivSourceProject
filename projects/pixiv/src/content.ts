...
export function getNovelInfo(res: any) {
  ...
  if (novel.seriesId && util.settings.IS_LEGADO) {
    ...
    let bookmarkId = JSON.parse(cache.get(`collect${novel.id}`) || "null");
...
export function getContent(res: any) {
  ...
  if (matched) {
    matched.forEach((pixivimage) => {
      ...
      if (pixivimage.includes("-")) {
        ...
        illustId = matched2?.[1] || 0;
        order = Number(matched2?.[2] || 0);
      } else {
        matched2 = pixivimage.match(RegExp("\\d+"));
        illustId = matched2?.[0] || 0;
      }
      content = content.replace(
        `${pixivimage}`,
  ...
  if (matched) {
    for (const i in matched) {
      const matched2 = matched[i].match(/\[\[jumpuri:(.*?)>(.*?)]]/m);
      const matchedText = matched2?.[0];
      const urlName = matched2?.[1].trim();
      const urlLink = matched2?.[2].trim();
      // 阅读不支持超链接
  ...
  if (matched) {
    for (const i in matched) {
      const matched2 = matched[i].match(/\[\[rb:(.*?)>(.*?)]]/m);
      const matchedText = matched2?.[0];
      const kanji = matched2?.[1].trim();
      const kana = matched2?.[2].trim() || "";
...
function getComment(res: any) {
  ...
  resp.body.comments.forEach((comment: any) => {
    ...
    if (comment.hasReplies === true) {
      ...
      resp.body.comments.reverse().forEach((reply: any) => {
        if (reply.comment === "") {
...
(() => {
  java.log("[DEBUG] load content.ts");
