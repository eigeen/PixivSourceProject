...
function buildPixivMainChannel(): BookSource[] {
  ...
  const loginUrlContent = readTextFile(path.join(distRoot, "base.loginUrl.js"));
  const loginCheckJsContent = readTextFile(path.join(distRoot, "base.loginCheckJs.js"));
  ...
  const discoverAddressContent = readTextFile(path.join(distRoot, "discover_address.js"));
  ...
  const bookUrlPattern = readTextFile(`${pixivRoot}/src/base.bookUrlPattern.txt`).trim();
  const loginUI = JSON.parse(readTextFile(`${pixivRoot}/src/base.loginUI.json`));
  const variableComment = readTextFile(`${pixivRoot}/src/base.variableComment.txt`);
